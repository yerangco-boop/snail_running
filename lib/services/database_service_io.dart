    ...
    return openDatabase(
      ...
      onCreate: (db, _) => db.execute('''
        CREATE TABLE workouts(
          ...
          weather_precipitation_percent INTEGER,
          route_json TEXT,
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 3) {
          await db.execute('ALTER TABLE workouts ADD COLUMN weather_temp_c REAL');
          await db.execute('ALTER TABLE workouts ADD COLUMN weather_humidity INTEGER');
          await db.execute(
              'ALTER TABLE workouts ADD COLUMN weather_precipitation_percent INTEGER');
        }
        if (oldVersion < 4) {
        ...
        if (oldVersion < 5) {
          await db.execute(
              'ALTER TABLE workouts ADD COLUMN lap_count INTEGER NOT NULL DEFAULT 0');
        }
        if (oldVersion < 6) {
          await db.execute('ALTER TABLE workouts ADD COLUMN lap_points_json TEXT');
        }
        if (oldVersion < 7) {
