...
- [ ] 메트로놈 `usageType`/`contentType` 변경 — 2026-07-11 커밋 메시지에 "과거 실기기 무음 버그의 근본 원인 수정을 되돌리는 것이라 사용자 확인 후 보류"라고만 기록되어 있고 원래 요청 내용 상세는 없음. **2026-09-07 v29에서 `metronomeDuringCall` 스위치로 해결됨** (아래 v29 항목 참고)
- [ ] **활동 종류 선택 (슬로우 조깅 / 러닝 / 걷기) — 2026-09-07 사용자 승인, "v29를 다음에 수정할 때 함께 진행" 으로 보류.** 기본값은 슬로우 조깅 유지.
  - 배경: "슬로우 조깅 말고 일반 러닝·산책·도보운동에서도 모든 측정이 정상이면 좋겠다"는 요청. 코드를 훑어 확인한 결과 **대부분의 측정은 활동 종류를 안 탐** — 시간·거리(GPS)·걸음수·케이던스·바퀴·경로·페이스 모두 정상. v28의 저정밀 속도 상한도 "최근 실측 속도 × 1.5" 방식이라 걷기 속도에 자동으로 맞춰짐. **손봐야 할 곳은 아래 2개뿐임**
  - **(1) 칼로리 MET 구간표** — `_metForPace`가 슬로우 조깅 기준으로 상향돼 있어(2026-08-12 v20) 걷기에서 크게 과다. 같은 페이스라도 조깅은 상하 운동이 커서 MET가 높은데 앱은 페이스만 보고 판단함. 검산값:

    | 활동 | 페이스 | 앱 MET | 실제 MET | 오차 |
    |---|---|---|---|---|
    | 러닝~슬로우 조깅 | 5~11분 | 6.2~10.0 | 6.0~11.5 | ±13% 이내 (문제 없음) |
    | 빠르게 걷기 | 10분 | 7.5 | 5.0 | **+50%** |
    | 보통 걷기 | 12분 | 6.2 | 3.5 | **+77%** |
    | 산책 | 15분 | 5.0 | 3.0 | **+67%** |

  - **(2) 케이던스 필터 통과대역** — 빠른 필터가 3Hz(`_stepFastAlpha=0.2738`)라 통과대역이 0.5~3.0Hz = **30~180spm**. 슬로우 조깅(150~175spm)은 여유가 있으나 **190~200spm대 전력 질주는 감쇠되어 걸음이 덜 잡힐 수 있음**(필터 설계상 추정, 실측 미확인). 러닝 모드에서는 4Hz(240spm)까지 확장 검토
  - 그 외: 걷기 모드에서는 BPM 권장 카드를 숨기는 게 자연스러움(표준 보폭표가 7~15분/km만 정의됨). 오래 서 있으면 GPS 잡음이 거리로 조금씩 쌓이는 문제는 활동 종류와 무관한 별개 사안
...
## 2026-09-14 v31 — 위치가 부정확할 때는 걸음 수 × 실측 보폭으로 거리·바퀴를 잰다

### 9/10·9/12·9/13 세 번의 로그로 확정한 사실
- **화면을 켜둔 구간은 완벽함**: 정확도 3~6m, 바퀴 거리 271~292m, 랩 시간 160~178초로 균일
- **화면이 꺼지면 측위가 무너짐**: 정확도 40~98m, 콜백 약 12초에 1번(와이파이·기지국 수준)
  - 지도 경로가 지그재그로 튐 (사용자 신고 "경로가 들쭉날쭉")
  - 거리 과다: 9/10 화면 끔 구간 1,805보 × 66.5cm = 1.20km인데 앱은 1.40km (+17%)
  - 가짜 바퀴: 9/10 랩10~16이 62~119초 간격 (정상 160~175초) → 16바퀴 기록, 실제 약 13바퀴
- 세 날 전체 추정 (걸음 × 화면 켬 구간 보폭): 9/10 3.79km(앱 4.02) / 9/12 3.74km(앱 4.25) / 9/13 3.79km(앱 4.17)
- **걸음 수는 화면이 꺼져도 150~160spm으로 안정**, 보폭은 세 날 모두 65~67cm로 일정

### geolocator 설정으로는 해결 불가 (소스 확인)
`forceLocationManager: true`는 LocationManager를 쓰지만 **안드로이드 12+에서는
LocationManager의 FUSED_PROVIDER를 다시 선택**함 (`LocationManagerClient.determineBestProvider`).
GPS 칩을 강제로 켜는 옵션이 아님 → 설정 변경이 아니라 앱 측 처리로 해결

### 처방 (home_screen.dart)
1. **보폭 학습**: 정확도 ≤12m, 간격 ≤5초, 시작 30초 이후, 걸음당 0.3~1.6m인 구간만 누적 (150보 이상부터 사용)
2. **걸음 기반 거리**: 좌표가 >20m이거나 직전 좌표가 부정확했거나 간격이 5초 초과면 → `걸음 수 × 학습 보폭`
   (걸음이 10초 안에 잡히고 있을 때만. 로그에 `걸음기반 N보->Xm (gps=Ym)` 표시)
3. **정밀 구간 튐 컷**: 걸음 수 × 보폭 × 1.6 + 1.5m를 넘는 GPS 이동은 잘라냄
4. **경로 표시**: 정확도 >20m 좌표는 경로선·스무딩에 넣지 않음 (부정확 구간은 직선으로 이어짐)
5. **바퀴**: 구간 전체가 정확했던 랩(첫 랩 제외)의 거리 중앙값을 학습 → 좌표가 부정확할 때는
   그 거리를 채울 때마다 한 바퀴(`[LAP] 바퀴 완료(거리 기반)`), 마커는 시작점에 표시.
   위치 기반 바퀴도 학습 거리의 60% 미만이면 보류 (복귀 직후 중복 방지)

### 9/14 실측 검증 — v31 정상 작동 확인 (47:17, 4.28km, 15바퀴, 6,844보)
- 보폭 학습: 러닝 1분 30초 만에 `보폭=67.4cm` → 이후 62.5cm로 수렴
- 한 바퀴 학습: 정밀 구간 랩 276·279·278·282·285·297·287m → 중앙값 282m
- 화면 꺼짐(19:38~19:58, 약 20분): 전 구간 `걸음기반` 누적, **거리 기반 바퀴 7개가 184~203초 간격**
  (화면 켬 구간 랩 171~197초와 같은 분포 — 9/10의 62~119초 가짜 바퀴 사라짐)
- 검산: 6,844보 × 62.5cm = 4.28km = 앱 기록, 15바퀴 × 282m = 4.23km (1% 차이)
- 경로 지도: 사각형 트랙 모양 깨끗이 표시, 지그재그 없음
- 페이스 11'03"는 이전(9'14")보다 느린데, 로그상 케이던스 145spm·보폭 62cm·랩 190초로
  세 지표가 일관되게 느려 **측정 오류가 아니라 실제로 천천히 달린 것**

### (v31 당시 작성) 다음 로그에서 확인할 것
- `보폭=66.xcm` 값이 러닝 1~2분 뒤 나타나는지, 화면 끔 구간에 `걸음기반` 줄이 찍히는지
- `[LAP] 한 바퀴 거리 학습 28xm`, 화면 끔 중 랩 시간이 160~180초로 균일한지
- 총 거리 ≈ 걸음 수 × 보폭, 바퀴 수 ≈ 거리 / 285m

## 2026-09-09 v30 — v28/v29 실측 검증: 남은 결함 3건 수정

### 검산 (코스 둘레 = 정밀 구간 랩당 286·286·289·286m → **약 287m**, 사용자 기억 280~290m와 일치)
| | |
|---|---|
| GPS 기록 | 4.06 km / 12바퀴 (겉보기 338m/랩) |
| 걸음 환산 (5,430걸음 × 69.2cm) | **3.76 km** |
| 13바퀴(누락 1 보정) × 287m | **3.73 km** ← 걸음 환산과 일치 |
| 결론 | 실제 약 3.75km, **과다 +8%** (v27의 +24%에서 개선), 실제 13바퀴 |

### (1) 바퀴 기준점이 acc=98.4m 측위로 잡혀 모든 랩이 30m 밀렸음
- 로그 첫 줄: `19:01:36 [GPS] 저정밀 모드 진입 (acc=98.4m)` → 이 포인트가 수락 #1이 되고
  **v28에서 랩 판정의 정확도 게이트를 없앤 탓에 그대로 `_lapStartPoint`가 됨**
- 증거: 랩별 최근접이 30.4 / 30.5 / 29.0 / 30.5 / 30.5 / 29.8m로 **일정** — 실제로는 출발선을
  몇 m 옆으로 스쳐 지나가는데도 그렇다는 건 기준점이 고정 오프셋만큼 어긋났다는 뜻
- 처방: **기준점만** `_lapAnchorMaxAccuracyMeters = 20m` 게이트를 두고, 잡힌 뒤의 판정에는
  v28대로 모든 포인트를 그대로 사용. `[LAP] 기준점 설정 (acc=..)` 로그 추가

### (2) 기준속도가 9.8km/h로 부풀어 저정밀 상한이 14.7km/h까지 열렸음
- v28의 EMA는 "정확도 ≤35m = 정밀"이면 전부 입력으로 받았는데, **10~35m 구간은 이미
  잡음이 커서 초당 속도가 9~24km/h로 튄다.** 9/9 로그에서 화면 꺼지기 직전 정확도가
  3→14m로 나빠지며 그 값들(9.1, 9.9, 21.4, 23.7…)이 EMA를 끌어올려 `기준속도=9.8km/h`가 됨
  (실제 6~7km/h) → 상한 14.7km/h → 잔여 과다 +8%의 원인
- 처방: EMA 폐기. **정확도 ≤12m 구간의 이동거리/시간 누적 평균**으로 교체
  (`_goodSpeedMeters / _goodSpeedSeconds`, 30초 이상 쌓이기 전엔 기본값 7.2km/h).
  한두 개 튄 값이 평균을 못 흔들고 데이터가 쌓일수록 실제 페이스에 수렴함

### (3) 통화 중 재생 스위치를 켜면 **오히려 무음**이었음
- 9/9 실측: 스위치 ON이면 통화 중이든 아니든 소리 없음, OFF면 남 → v29에서 고른
  `assistanceSonification`이 타는 **알림/시스템 볼륨이 이 기기에서 사실상 꺼져 있음**
  (2026-07-03에 media로 바꿨던 바로 그 원인)
- 처방: 통화용 경로를 **알람 스트림(`AndroidUsageType.alarm`)** 으로 교체 — 통화 중에도
  울리도록 설계된 채널이고 알람 볼륨을 0으로 두는 경우는 드묾
- **설정 카드에 "소리 테스트" 버튼 추가** — 현재 스트림/음량/BPM 그대로 3초간 울려봄.
  러닝을 시작하거나 전화가 올 때까지 기다리지 않고 그 자리에서 검증 가능
  (이번에도 러닝 중에야 무음을 발견했음)

### (4) 워치독 45→30초
화면 꺼짐 중 콜백이 55초간 완전히 끊겨(`콜백 자체가 끊김` 로그) 그 사이 한 바퀴가 통째로
누락됨(랩3이 391초 = 정상 164초의 2.4배). 재시작 비용이 작으므로 더 일찍 개입하게 함

### 잘 작동한 것
화면 꺼짐 중 **거리·걸음 정상 누적**(사용자 확인), 정밀 구간 랩당 286~289m로 안정,
워치독이 콜백 중단을 정확히 진단

## 2026-09-07 v29 — "통화 중 메트로놈 무음"은 정상 동작이 아니라 **우리가 바꾼 것이었음**

v27~v28에서 "통화 중 무음은 안드로이드 정상 동작"이라고 두 번 단정했는데 **틀렸음.**
사용자가 "예전에는 통화 중에도 들렸다"고 반박해 git 이력을 뒤진 결과 원인이 특정됨:

- **커밋 `095b053` (2026-07-03)** 에서 오디오 출력 스트림을 바꿨음
  - `contentType: sonification → music`
  - `usageType: assistanceSonification → media`
- `assistanceSonification`(알림음 계열)은 **통화 중에도 시스템이 막지 않음**.
  `media`는 **통화 중 차단됨** → 7/3 이후로 통화 중 무음이 된 것
- 그 변경 자체는 이유가 있었음: sonification 계열은 미디어 볼륨이 아니라 별도의
  알림/시스템 볼륨을 타서, 그 볼륨이 0이면 **아예 안 들리는** 실기기 버그가 있었음

즉 **트레이드오프이지 정상 동작이 아니었음.** 어느 쪽이 맞는지는 사용 상황에 달려 있어
`AppSettings.metronomeDuringCall` 스위치(기본 false)로 사용자가 고르게 함.
- 스위치가 오디오 스트림 종류 자체를 바꾸므로 `didUpdateWidget`에서 mix 설정과 같은
  경로로 재초기화 → **통화 중에 스위치를 켜도 그 자리에서 반영됨**
- 설정 카드에 "소리가 작으면 미디어 볼륨이 아니라 알림 볼륨을 올리라"는 안내를 함께 표시

**교훈**: "OS 표준 동작이라 어쩔 수 없다"고 말하기 전에 **git 이력에서 우리가 바꾼 게
아닌지 먼저 확인할 것.** 사용자의 "예전엔 됐는데"는 무시하면 안 되는 증거임.

## 2026-09-07 v28 — v27이 "유실"을 "과다"로 바꿔놓았음 (저정밀 구간 보정)

9/7 실측으로 v27 검증. **거리 유실은 해결됐지만 두 가지 새 문제가 드러남.**

### 랩 거리로 검산한 결과 (코스 실측 둘레 ≈ 296m)
| 구간 | m/랩 |
|---|---|
| 화면 켬 (랩1~2) | **296m** ← 기준 |
| 랩4~5 (저정밀 포함) | 626m |
...
- 저정밀 세그먼트 개별 속도가 **17~24km/h**로 찍힘. 슬로우 조깅 실제 속도는 6~8km/h이므로
  이건 달린 게 아니라 **측위 잡음이 그대로 거리로 들어간 것**. `_maxPlausibleSpeedKmh=25`
  상한이 슬로우 조깅에는 너무 헐거워서 잡음을 거의 못 걸렀음
- 처방: 저정밀 구간은 **최근 정밀 구간에서 측정된 실제 속도(`_recentSpeedMps`, EMA)의 1.5배**를
  넘지 못하게 추가로 자름. 멈춰 있으면 짧은 값이 그대로 쓰이므로 과소 위험은 없음.
  로그에 `기준속도=6.8km/h` 병기

### 바퀴만 멈춘 원인 = v27에서 내가 넣은 50m 게이트
- v27의 `_lapMaxAccuracyMeters = 50`이 저정밀 포인트를 **바퀴 판정에서 제외**했는데,
  화면 꺼짐 구간의 정확도가 56~98m라 **바퀴 판정에 들어가는 포인트가 아예 없었음**
- 실제로 랩3이 311초(정상 165초의 약 2배)로 찍혀 그 사이 한 바퀴가 통째로 누락됨
- 처방: **버리지 말고 판정 반경을 그 시점 측위 오차만큼 넓힘** —
  접근 반경 `max(32m, accuracy)`, 통과 확인 `max(8m, accuracy×0.25)`.
  중복 카운트는 기존 60초 최소 간격이 계속 막아줌

### v27에서 확인된 것
- **거리 누적은 화면 꺼짐 중에도 정상 작동** (`[GPS](저정밀)` 줄로 확인). v27 핵심 수정 유효
- `[GPS] 워치독`이 실제로 2회 발동했고 `콜백 자체가 끊김`으로 원인을 정확히 구분해 기록함
  → 화면 꺼짐 시 **정확도 저하와 콜백 중단이 둘 다** 일어난다는 것이 확정됨
- 통화 중 메트로놈 무음 → 통화 종료 시 즉시 복구: **정상 동작 재확인** (수정 대상 아님)

## 2026-09-06 v27 — 화면 꺼짐 거리 유실의 진짜 원인: **우리 필터가 버리고 있었음**

**v25의 진단(“OS가 위치 콜백을 억제한다”)은 틀렸음.** 9/6 로그에 `[LIFECYCLE]`이 찍히면서
처음으로 화면 꺼짐 구간과 GPS 수신을 1:1로 대조할 수 있게 됐고, 사실은 정반대였음:

- **화면이 꺼져도 위치 콜백은 초당 1회로 멀쩡히 들어온다.** 끊긴 적이 없음
- 대신 **정확도가 계단식으로 나빠진다**: 3m → 6 → 15 → 30 → 38 → 116m (약 30~90초에 걸쳐).
  OS가 화면 꺼짐 상태에서 GNSS 듀티를 낮추고 저전력 측위로 내려가는 것
- 그런데 기존 코드는 `accuracy > 35m`면 **모든 포인트를 통째로 버리고 `return`** 했음
  → 거리·바퀴가 그대로 얼어붙고, 화면을 켜는 순간 그동안의 이동이 직선 한 방(103m, 180.5m)으로 합쳐짐
- **워치독도 못 잡았음**: `_lastGpsCallbackAt`을 정확도 검사 *전에* 갱신해서, 콜백이 오는 한
  워치독은 “스트림 정상”으로 판단했음. 그래서 로그에 `[GPS] 워치독` 줄이 한 번도 안 나옴
- 실측 유실량: 화면 끔 5분 구간 = 실제 약 460m 중 292m만 누적(-168m),
  화면 끔 3분 구간 = 실제 약 290m 중 33m만 누적(-257m)

### 처방 (v27)
- **정확도 3단계로 분리**: ≤35m 정밀(기존대로 매 콜백 누적) / 35~100m **저정밀(버리지 않고 누적)** / >100m만 폐기
- 저정밀 구간은 1초 이동(약 1.7m)이 잡음에 묻히므로 **12초 이상 + 10m 이상 이동했을 때만** 직선거리로 한 번에 반영.
  실제 경로보다 조금 짧게 잡히지만 통째로 0이 되는 것보다 훨씬 정확함
- **바퀴 판정은 50m 이하 포인트만** 사용 (40m 아웃존/8m 통과확인이라 잡음에 더 민감)
- **워치독 기준을 “마지막 채택 측위”로 변경**하고, `콜백은 오는데 정확도 미달`과 `콜백 자체가 끊김`을 로그에서 구분
- `[GPS] 저정밀 모드 진입/정밀 모드 복귀` 로그 추가, 누적 로그에 `(저정밀)` 표시

### 통화 중 메트로놈 무음은 정상 동작 (수정 대상 아님)
9/6 확인: **통화 중에는 무음, 끊으면 소리 복귀, 다시 통화되면 무음**. 이는 안드로이드가 통화 중
미디어 오디오를 막는 정상 동작이고, v25의 “통화 후 복구” 수정은 제대로 작동하고 있는 것.
통화 중에도 소리를 내려면 통화 오디오 스트림을 가로채야 해서 하지 않음.

### v25/v26에서 실제로 잘 작동한 것 (로그로 확인)
- **바퀴 9회 연속 카운트** (최근접 6.6~11.9m, 구간 176~184초로 균일) — CPA 전환 유효.
  9/4에 4바퀴에서 멈추던 문제 해결됨
- 러닝 중 BPM 165→175 즉시 반영됨
- 메트로놈 음량 0.71 유지 (하한 도입 후 0.0 함정 재발 없음)
- 알림 권한·배터리 최적화 예외 모두 granted

## 2026-09-05 v25 — 9/4·9/5 실측 로그로 확정한 결함 4건 수정

9/4·9/5 두 세션의 디버그 로그를 대조해 **증상이 아니라 로그상의 근본 원인**을 먼저 확정한 뒤 수정함.

- [x] **통화 후 메트로놈이 영영 안 살아나던 문제 (근본 원인 확정)** — `ClickPlayer._player`가 `final AudioPlayer`라 **인스턴스를 한 번도 새로 만들지 않았음**. `AudioFocus.none`(음악 앱과 겹쳐 재생) 때문에 전화가 오면 시스템이 SoundPool 스트림을 회수하는데 포커스 복귀 통지를 못 받고, 그 죽은 인스턴스에 `setSource`/`setVolume`을 다시 호출해봐야 되살아나지 않음. 9/4 로그에 `[Metro] 재초기화 완료`가 5번 찍혔는데도 소리가 안 난 것이 이 증거(재초기화 로그는 남는데 실제로는 아무것도 복구되지 않았음). → `init()`에서 기존 플레이어를 `release()`+`dispose()` 후 **새 AudioPlayer로 교체**. `MetronomeService.init()`도 먼저 `stop()`하도록 하고, 폐기된 플레이어에 남은 `play()`가 도달해도 죽지 않게 try/catch 추가
- [x] **BPM을 바꿔도 박자가 안 바뀌던 문제** — `didUpdateWidget`이 TTS 음성/mix 설정/음량만 감시하고 **`bpm`과 `metronomeEnabled`는 아예 안 봤음**. `MetronomeService.updateBpm()`은 만들어만 두고 어디서도 호출되지 않는 죽은 코드였음. → `_appliedBpm`/`_appliedMetronomeEnabled` 비교값을 추가해 설정 화면에서 BPM을 바꾸거나 "권장 BPM 적용"을 누르면 러닝 중에도 즉시 반영. (목표 페이스는 BPM을 자동으로 바꾸지 않음 — 페이스→BPM은 v22부터 "권장 + [적용] 버튼" 방식이 의도된 동작)
- [x] **메트로놈 음량 0 함정** — 9/5 로그의 `[Metro] 재초기화 완료 (bpm=175, vol=0.0)`. 설정 화면을 스크롤하다 슬라이더 왼쪽 끝을 건드리면 음량이 0이 되는데, 그 뒤로는 **켜져 있고 BPM도 바꿔지지만 아무 소리도 안 나서** "무엇을 해도 반응이 없는" 상태가 됨. 사용자가 겪은 "on/off 해봐도 무반응"의 정체. → `kMinMetronomeVolume = 0.15` 하한 도입(슬라이더 min + load 시 clamp). 무음이 필요하면 메트로놈 스위치를 끄면 됨
- [x] **바퀴 카운트가 4바퀴에서 멈춘 문제 (구조적 결함)** — 9/4 로그의 바퀴별 복귀 이격거리가 **17.2 → 13.4 → 16.2 → 17.0m**로 복귀 반경 18m 경계에 붙어 있다가 5바퀴째에 넘어갔고, 그 뒤 25분을 더 돌았는데도 하나도 안 잡힘(총 4바퀴로 종료). GPS 산포(정확도 5~15m)가 고정 반경보다 커지는 순간 반드시 실패하는 구조 — 9/5에 10바퀴가 정상으로 나온 건 운 좋게 반경 안에 들어왔을 뿐. → **고정 반경 판정을 폐기하고 최근접 통과(CPA) 판정으로 교체**: 40m 아웃존을 벗어난 뒤 시작점에 가장 가까워진 지점을 추적하다가, 그 최근접점보다 8m 이상 다시 멀어지면 "지나갔다"로 확정. 기준이 상대값이라 GPS가 몇 미터 밀려 있어도 성립함. 랩 기록 시각·거리는 최근접 시점의 값을 사용
- [x] **화면을 끄면 거리·바퀴가 멈추던 문제** — 9/5에 사용자가 전원 버튼으로 화면을 끈 구간(19:19:37~19:23:43 등)에서 **가속도계는 10초마다 정상 수신(= 프로세스는 살아 있음)인데 위치 콜백만 4분 넘게 끊겼고**, 화면을 켠 순간 그 구간이 직선 103m 한 방으로 합쳐져 실제 주행 경로만큼의 거리가 사라짐. 즉 프로세스 종료나 알고리즘 문제가 아니라 **OS의 백그라운드 위치 억제**이고, 포그라운드 서비스가 실효를 못 내고 있다는 뜻.
  - **중요**: v18에서 거리 오차가 -2.4%로 좋아진 건 포그라운드 서비스 덕이라고 기록해뒀지만, 그 테스트들은 `keepScreenOn` 기본 ON이라 **화면이 계속 켜져 있었음** — 실제 백그라운드는 이번에 처음 시험된 것이고 실패했음. 기존 기록의 해석을 이렇게 정정함
  - 처방: ①`permission_handler` 도입 → 러닝 시작 시 **알림 권한(Android 13+)**과 **배터리 최적화 예외**를 한 번씩 요청(`lib/services/background_permissions.dart`, 매니페스트에 `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` 추가). 삼성 "앱 절전"이 유력 용의자 ②`AndroidSettings.intervalDuration`을 1초로 명시 ③**GPS 워치독**: 25초 이상 위치 콜백이 없으면 스트림을 다시 걸고 `[GPS] 워치독` 로그 기록 ④위치 스트림에 없던 `onError`/`onDone` 핸들러 추가(조용히 끊겨도 로그가 남게) ⑤`[LIFECYCLE]` 로그 추가 — **다음 로그에서는 "화면이 꺼져 있던 시간대"와 "위치가 안 들어온 시간대"를 직접 대조할 수 있음**

### QR코드가 아직도 v14를 가리키고 있었음 (2026-09-05 확인)
v25 설치 시 "패키지가 잘못되어 앱이 설치되지 않았습니다"가 떠서 확인한 결과, **파일 손상이 아니라
QR이 옛 태그 주소에 묶여 있어 v14(51.15MB)를 받고 있었음.** 폰에 v24가 깔린 상태에서 versionCode 14를
설치하려 하니 버전 다운그레이드로 거부된 것이고, 삼성은 이때 위 메시지를 띄움.
- **다운로드 파일 크기로 버전을 즉시 판별할 수 있음**: v13/14=51.15MB, v17~20=52.4~52.9MB,
  v21~23=53.32MB, v24=55.42MB, v25=55.50MB, v30=55.51MB, v31=55.51MB(55,514,939바이트), v32=55.53MB(55,531,323바이트), v33=55.53MB(55,531,323바이트 — v32와 바이트까지 동일, 버전 표시 `1.1.4 (33)`로만 구분), v34=55.53MB(55,531,419바이트)
  — **v30과 v31은 폰 표시 크기가 같아 구분 불가** → 설정 화면 하단 버전(`1.1.2 (31)`)으로 확인할 것
- **단위 주의 (2026-09-09 정정)**: 사용자 폰의 다운로드 표시는 **십진 MB(÷1,000,000)** 임.
  55,514,935바이트 → **55.51MB**로 보임. 위 표가 곧 폰에 보이는 숫자 그대로임.
  이전에 "안드로이드는 MiB로 표시하므로 52.94MB로 보인다"고 적어뒀던 것은 **오기**였음 —
  2026-09-05에 사용자가 본 "51.15MB"가 v14의 51,151,459바이트와 십진으로 정확히 일치했던
  것이 이미 반증이었는데 그걸 놓치고 반대로 기록했음. **MiB로 환산해서 안내하지 말 것**
- 2026-09-05에 `latest/download` 고정 주소로 QR을 재발급하고 **opencv로 디코딩해 주소까지 검증함**.
  재발급 스크립트는 위 "환경" 섹션 참고. PNG는 `.gitignore` 대상이라 커밋되지 않음
- 앞으로 설치 실패 신고가 오면 **APK 무결성보다 "받은 파일 크기 = 최신 릴리스 크기"를 먼저 확인할 것**

### 다음 실외 테스트에서 확인할 것 (v25)
1. 러닝 시작 시 **알림 권한 / 배터리 최적화 예외** 다이얼로그가 뜨는지, 허용했는지
2. 알림창에 **"달팽이 러닝 / 러닝 기록 중"** 이 뜨는지 (안 뜨면 포그라운드 서비스가 안 붙은 것)
3. **전원 버튼으로 화면을 끄고 3~5분 달린 뒤** 그 구간 거리·바퀴가 정상 누적되는지 → 안 되면 로그의 `[LIFECYCLE]`·`[GPS] 워치독` 줄을 볼 것
4. 통화를 받았다 끊은 뒤 메트로놈이 자동 복구되는지
5. 러닝 중 설정에서 BPM을 175→150으로 바꿨을 때 박자가 즉시 느려지는지
6. 코스를 6바퀴 이상 돌아도 바퀴가 계속 카운트되는지 (`[LAP] 최근접=` 값이 얼마까지 벌어지는지)

## 2026-09-01 세션 종료 시점 (versionCode 22 배포 완료)

### 오늘 마무리된 것
- v22 APK/웹 빌드 모두 성공. BPM 제안을 슬로우 조깅 표준 보폭 기준으로 재설계 완료

### 내일 첫 번째로 할 일 — 디자인 정확도 문제
**진단 확정**: 지금까지 디자인이 실패한 이유는 컨셉 불일치가 아니라 **정확도(fidelity)**였음.
사용자 확인: "결과물이 이미지와 안 닮았어". 원인 두 가지 —
1. 이미지에서 색상 hex·간격 px·폰트 크기를 **눈대중으로 추정**해서 오차가 누적됨
2. **Claude가 자기 결과물을 한 번도 못 봄** — 코드 작성 후 바로 배포라 시안과 비교·수정하는 과정이 아예 없음

**중요한 발견 (2026-09-01)**: `https://yerangco-boop.github.io/snail_running/` **GitHub Pages 웹 빌드가 실제로 살아있음**(200, flutter_bootstrap.js 확인). 매 커밋마다 자동 배포되고 있었는데 그동안 아무도 안 써봄. 즉 **노트북에 Flutter가 없어도 Claude가 배포된 앱을 브라우저로 열어 자기 결과를 확인할 수 있음** — 원인 2번을 해결할 경로가 이미 갖춰져 있었음.
- 기존 "GitHub Pages 주소에서 웹 빌드 동작 테스트" 미완료 항목은 이걸로 확인됨

**막고 있는 것**: 작업 규칙의 "브라우저 자동 스크린샷 시도 금지" 조항. 이 규칙이 위 해결책을 정확히 금지하고 있어 **사용자 결정 대기 중**(2026-09-01 밤에 시간이 늦어 보류). 선택지는 ①전면 해제 ②디자인 작업 중에만 허용 ③유지하고 Figma 등 정확한 수치 전달로만 해결.

**디자인 소스 경로도 미결정** — 선택지: Figma Dev Mode MCP 수동 설정(정확한 수치를 구조화 데이터로 전달, 추정 오차 제거) / 이미지 재시도(스크린샷 자가검증이 가능해진 전제하에) / FlutterFlow 등이 Flutter 코드를 직접 생성하고 Claude는 통합만.
- MCP 레지스트리 검색 결과 **현재 연결 가능한 디자인 커넥터 없음** — Figma는 수동 설정 필요. Canva MCP는 이 프로젝트 세션 초반에 붙어 있다가 끊김

### v22 실기기 확인 대기 항목
- 설정 → 목표 페이스 아래 "이 페이스엔 175 BPM 권장 / 보폭 54cm로 잔발 (지난 러닝 62cm)" 카드
- 통화 후 메트로놈 자동 복구, 랩 표의 실측 누적거리, DB v9 마이그레이션 후 기존 이력 보존
